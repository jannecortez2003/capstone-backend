...
const multer = require('multer'); // Added for ID image uploads
const fs = require('fs');
...
// Serve uploaded images publicly so the Admin can see them
...
app.post('/adminlogin', (req, res) => {
  ...
  db.query("SELECT * FROM admins WHERE username = ?", [username], (err, results) => {
    ...
    if (password === results[0].password) res.json({ success: true, message: "Login successful", admin: { id: results[0].id, username: results[0].username, role: 'admin' } });
    else res.status(400).json({ success: false, message: "Invalid password" });
  });
});
...
app.post('/book_event', (req, res) => {
  ...
  db.query(`INSERT INTO appointments (user_id, event_type, package_type, preferred_date, guest_count, selected_dishes, notes, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'Pending')`, 
...
app.post('/admin_update_booking_status', (req, res) => {
  ...
  db.query(sql, [status, bookingId], () => res.json({ success: true, message: "Status updated" }));
});
...
app.post('/admin_add_inventory', (req, res) => db.query("INSERT INTO inventory (name, quantity, unit) VALUES (?, ?, ?)", [req.body.name, req.body.quantity, req.body.unit], () => res.json({ success: true })));
app.post('/admin_update_inventory', (req, res) => db.query("UPDATE inventory SET name=?, quantity=?, unit=? WHERE id=?", [req.body.name, req.body.quantity, req.body.unit, req.body.id], () => res.json({ success: true })));
app.post('/admin_delete_inventory', (req, res) => db.query("DELETE FROM inventory WHERE id = ?", [req.body.id], () => res.json({ success: true })));

// Menu
app.get('/admin_fetch_menu', (req, res) => db.query("SELECT * FROM menu_items", (err, r) => res.json({ success: true, items: r })));
app.post('/admin_add_menu', (req, res) => db.query("INSERT INTO menu_items (name, category, price, description) VALUES (?, ?, ?, ?)", [req.body.name, req.body.category, req.body.price, req.body.description], () => res.json({ success: true })));
app.post('/admin_update_menu', (req, res) => db.query("UPDATE menu_items SET name=?, category=?, price=?, description=? WHERE id=?", [req.body.name, req.body.category, req.body.price, req.body.description, req.body.id], () => res.json({ success: true })));
app.post('/admin_delete_menu', (req, res) => db.query("DELETE FROM menu_items WHERE id = ?", [req.body.id], () => res.json({ success: true })));
...
app.post('/admin_delete_staff', (req, res) => db.query("DELETE FROM staff WHERE id = ?", [req.body.id], () => res.json({ success: true })));
...
app.get('/admin_fetch_payment_history', (req, res) => {
  db.query("SELECT * FROM payments WHERE appointment_id = ? ORDER BY transaction_date DESC", [req.query.appointmentId], (err, r) => res.json({ success: true, history: r }));
});
app.post('/admin_process_payment', (req, res) => {
  db.query("INSERT INTO payments (appointment_id, amount_paid, payment_type, remarks) VALUES (?, ?, ?, ?)", [req.body.appointmentId, req.body.amount, req.body.paymentType, req.body.remarks], () => res.json({ success: true }));
});
...
app.get('/admin_fetch_verification', (req, res) => {
  db.query("SELECT * FROM user_verifications WHERE status = 'Pending'", (err, r) => res.json({ success: true, requests: r }));
});
app.post('/admin_verify_user', (req, res) => {
  const { requestId, status } = req.body;
  db.query("UPDATE user_verifications SET status = ? WHERE id = ?", [status, requestId], (err) => {
    if (status === 'Verified') db.query("UPDATE users SET is_verified = 1 WHERE id = (SELECT user_id FROM user_verifications WHERE id = ?)", [requestId]);
